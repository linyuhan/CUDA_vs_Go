...
func (apop *Tpop) chernobyl2(sizepop int) {
...
func main() {
	...
	counter := 0
	migracao := 1
	for Gpop1[0].v > 0.0001 || Gpop2[0].v > 0.0001{
		
			go Gpop1.chernobyl2(Quantos)
				
			go Gpop2.chernobyl2(Quantos)
			
			
		
		if (Gpop1[0].v < 0.001 || Gpop2[0].v < 0.001) && migracao==1{ //migração
			fmt.Println("migracao: ", counter)
			aux = Gpop1[0]
			Gpop1[0] = Gpop2[0]
			Gpop2[0] = aux
			migracao = 0
